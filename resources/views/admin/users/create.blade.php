    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone_country_code') ? 'is-invalid' : '' }}" type="text" name="phone_country_code" id="phone_country_code" value="{{ old('phone_country_code', '+965') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('otp_expires_at') ? 'is-invalid' : '' }}" type="text" name="otp_expires_at" id="otp_expires_at" value="{{ old('otp_expires_at') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('otp_attempts') ? 'is-invalid' : '' }}" type="number" name="otp_attempts" id="otp_attempts" value="{{ old('otp_attempts', '0') }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('otp_consumed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="otp_consumed" id="otp_consumed" value="1" {{ old('otp_consumed', 0) == 1 || old('otp_consumed') === null ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('otp_channel') ? 'is-invalid' : '' }}" name="otp_channel" id="otp_channel">
                    ...
                    @foreach(App\Models\User::OTP_CHANNEL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('otp_channel', 'sms') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\User::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', 'talent') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
