...
<div class="outfit-form-shell">
    <div class="top-row">
        <div class="title-block">
            <h5>Add New Outfit</h5>
            <div class="sub">Create a new outfit for the system.</div>
        </div>
        ...
        <form method="POST" action="{{ route("admin.outfits.store") }}" enctype="multipart/form-data">
            ...
            <div class="field-block">
                ...
                <div class="dark-input {{ $errors->has('name') ? 'is-invalid' : '' }}">
                    <input type="text" name="name" id="name" value="{{ old('name', '') }}" placeholder="Enter outfit name" required>
                </div>
                @if($errors->has('name'))
            ...
            <div class="category-row">
                ...
                <div class="field-block">
                    ...
                    <select name="sub_category" id="sub_category" class="light-select {{ $errors->has('sub_category') ? 'is-invalid' : '' }}" required>
                        <option value="" disabled {{ old('sub_category', null) === null ? 'selected' : '' }}>Please select</option>
                        @foreach(App\Models\Outfit::SUB_CATEGORY_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('sub_category', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div class="field-block">
                ...
                <div class="dark-input {{ $errors->has('sort_order') ? 'is-invalid' : '' }}">
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', '0') }}" placeholder="0" step="1">
                </div>
