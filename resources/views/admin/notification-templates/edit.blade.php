...
<div class="content">
    <div class="shoot-header">
        <div>
            <h2 class="shoot-title1">Edit Notification Template</h2>
            <p class="shoot-subtitle">Update system notification titles and messages</p>
        </div>
    ...
    <div class="card border-0 shadow-sm mt-4">
        <div class="card-body bg-white p-4">
            <form method="POST" action="{{ route('admin.notification-templates.update', [$notificationTemplate->id]) }}" id="templateEditForm">
                @method('PUT')
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input class="form-control border-dark" type="text" name="title_en" id="title_en" value="{{ old('title_en', $notificationTemplate->title_en) }}">
                        </div>
                        <div class="form-group mb-3">
                            <label class="font-weight-bold text-dark text-uppercase text-xs mb-1" for="content_en">Message Content (English)</label>
                            <textarea class="form-control border-dark" name="content_en" id="content_en" rows="5">{{ old('content_en', $notificationTemplate->content_en) }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input class="form-control border-dark" type="text" name="title_ar" id="title_ar" value="{{ old('title_ar', $notificationTemplate->title_ar) }}" style="direction: rtl; text-align: right;">
                        </div>
                        <div class="form-group mb-3">
                            ...
                            <textarea class="form-control border-dark" name="content_ar" id="content_ar" rows="5" style="direction: rtl; text-align: right;">{{ old('content_ar', $notificationTemplate->content_ar) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group mb-4">
                            ...
                            <select class="form-control border-dark" name="language_preference" required>
                                <option value="both" {{ old('language_preference', $notificationTemplate->language_preference) == 'both' ? 'selected' : '' }}>Both English & Arabic</option>
                                <option value="en" {{ old('language_preference', $notificationTemplate->language_preference) == 'en' ? 'selected' : '' }}>English Only</option>
                                <option value="ar" {{ old('language_preference', $notificationTemplate->language_preference) == 'ar' ? 'selected' : '' }}>Arabic Only</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4 d-flex align-items-center mb-4">
                        <div class="custom-control custom-switch">
                            ...
                            <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" value="1" {{ old('is_active', $notificationTemplate->is_active) == 1 ? 'checked' : '' }}>
    ...
    $(function() {
        $('#templateEditForm').on('submit', function(e) {
            ...
            Swal.fire({
                ...
                text: "Are you sure you want to update this notification template?",
                icon: 'question',
