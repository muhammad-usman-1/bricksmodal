...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.castingApplication.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.casting-applications.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('casting_requirement') ? 'is-invalid' : '' }}" name="casting_requirement_id" id="casting_requirement_id" required>
                    @foreach($casting_requirements as $id => $entry)
                        <option value="{{ $id }}" {{ old('casting_requirement_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('talent_profile') ? 'is-invalid' : '' }}" name="talent_profile_id" id="talent_profile_id" required>
                    @foreach($talent_profiles as $id => $entry)
                        <option value="{{ $id }}" {{ old('talent_profile_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rate_offered') ? 'is-invalid' : '' }}" type="number" name="rate_offered" id="rate_offered" value="{{ old('rate_offered', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('talent_notes') ? 'is-invalid' : '' }}" name="talent_notes" id="talent_notes">{{ old('talent_notes') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\CastingApplication::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'applied') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rating') ? 'is-invalid' : '' }}" type="number" name="rating" id="rating" value="{{ old('rating', '0') }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('reviews') ? 'is-invalid' : '' }}" name="reviews" id="reviews">{{ old('reviews') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payment_processed') ? 'is-invalid' : '' }}" name="payment_processed" id="payment_processed" required>
                    ...
                    @foreach(App\Models\CastingApplication::PAYMENT_PROCESSED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payment_processed', 'n/a') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
