...
<div class="modal-shell">
    <div class="modal-card">
        <div class="modal-head">
            <div>
                <p class="modal-title">Add User Permissions</p>
                ...
                <div>
                    ...
                    <div class="row-2">
                        <div>
                            ...
                            <input class="input {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Admin" required>
                            @if($errors->has('name'))
                        ...
                        <div>
                            ...
                            <input class="input {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email') }}" placeholder="admin@admin.com" required>
                            @if($errors->has('email'))
                    ...
                    <div class="row-2">
                        <div>
                            ...
                            <select class="select {{ $errors->has('role_id') ? 'is-invalid' : '' }}" name="role_id" id="role_id" required>
                                ...
                                @foreach($roles as $role)
                                    ...
                                    <option value="{{ $role->id }}" data-perm-count="{{ $permCount }}" {{ old('role_id') == $role->id ? 'selected' : '' }}>{{ ucfirst($role->title) }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input class="input {{ $errors->has('password') ? 'is-invalid' : '' }}" type="password" name="password" id="password" placeholder="Enter password" required>
                    ...
                    <div class="row-2">
                        <div></div>
                        <div>
                            <label class="label required" for="password_confirmation">Confirm Password</label>
                            <input class="input" type="password" name="password_confirmation" id="password_confirmation" placeholder="Confirm password" required>
                        </div>
                    </div>
                </div>
